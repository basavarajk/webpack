...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		if(options.optimization.minimize) {
			const minimizers = [].concat(options.optimization.minimizer).map(m => {
				if(typeof m === "string") {
					return {
						name: m
					};
			...
			const moduleHelper = new Module("", null);
			moduleHelper.paths = Module._nodeModulePaths(options.context);
			for(const minimizer of minimizers) {
				const Plugin = moduleHelper.require(minimizer.name);
				compiler.apply(new Plugin(minimizer.options));
			}
		}
